<script lang="ts">
	...
	let newCon = Array(columns.length).fill('');

	function addCon() {
        if (Connection.parseTime(newCon[1]) === -1) {
		...
		if (!Connection.isValidModeStr(newCon[4])) {
			...
			{#each connections as c, index}
				<tr>
					...
					<td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
						<input
							type="number"
							bind:value={c.end_length}
							class="border-0 w-16"
							on:change={onChangeConnections}
						/>
					</td>
					<td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        {#each c.dominated as d,index}
                        [{d[0]}, {Math.round(d[1])}]<br>
                        {/each}
					</td>
                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                        {#if c.dominated.length === 0 && c.closest_to_dominate_index !== -1}
                        [{connections[c.closest_to_dominate_index].name}, {Math.round(c.closest_to_dominate_res)}]<br>
                        {/if}
					</td>
